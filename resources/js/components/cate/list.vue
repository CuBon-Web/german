          ...
          <div class="card">
            <div class="card-body">
              ...
              <vs-table max-items="10" pagination :data="list">
                <template slot="thead">
                  <vs-th>ID</vs-th>
                  <vs-th>Vị trí hiển thị</vs-th>
                  <vs-th>Tên</vs-th>
                  <vs-th>Vị trí hiển thị slide trang chủ</vs-th>
                ...
                <template slot-scope="{data}">
                  <vs-tr :key="indextr" v-for="(tr, indextr) in data">
                    ...
                    <vs-td :data="tr.id">{{tr.order_id}}</vs-td>
                    <vs-td :data="tr.name">{{JSON.parse(tr.name)[0].content}}</vs-td>
                    <vs-td :data="tr.id" v-if="tr.location_banner == 1">
                      Bên Trái
                    </vs-td>
                    <vs-td :data="tr.id" v-if="tr.location_banner == 0">
                      Bên Phải
                    </vs-td>
                    <vs-td :data="tr.id">
